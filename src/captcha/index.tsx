...
export default class Captcha extends React.Component {
  ...
  render() {
    ...
    if (type === 'popup') {
      return (
        <Pop panel={this.props.panel} visible={visible} onClose={this.onClose}>
          ...
          {captchaType === 'point' && (
            <Point {...options} onSuccess={this.onSuccess} />
          )}
        </Pop>
      );
    }
    return (
      <Fragment>
        {(captchaType === 'auto' || captchaType === 'slide') && (
          <Slide {...options} onValidFail={this.onFail} />
        )}
        {captchaType === 'point' && <Point {...options} />}
      </Fragment>
    );
  }
}
